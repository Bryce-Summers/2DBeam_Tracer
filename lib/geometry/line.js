...
(function() {
  BT2D.Line = (function() {
    ...
    Line.prototype.intersectFrustrum = function(lightFrustrum, min_time1, min_time2) {
      var bp_left, bp_right, bp_start, c_l_l, c_l_r, c_r_l, c_r_r, dist1, dist2, end1, end2, focus, frustrum, i_left, i_right, intersection1, intersection2, intersection_exists, intersection_left, intersection_right, left_frustrum, pt_left, pt_left2, pt_right, pt_right2, ray1, ray2, ref, right_frustrum, t_left, t_right;
       ...
       */
      ref = this._radiallyOrientPts(lightFrustrum, this._p1, this._p2), pt_left = ref[0], pt_right = ref[1];
      pt_left2 = pt_left;
      pt_right2 = pt_right;
    ...
    Line.prototype.split = function(bp) {
      ...
      if (this._type !== BT2D.Line.UNBOUNDED && time < BT2D.Constants.EPSILON) {
        if (side2 < 0) {
          return [this, null, null];
        }
        if (side2 === 0) {
      ...
      if (this._type === BT2D.Line.SEGMENT && time > 1.0 - BT2D.Constants.EPSILON) {
        if (side1 < 0) {
          return [this, null, null];
        }
        if (side1 === 0) {
    ...
    Line.prototype.getPerpendicularPercentage = function(dir) {
      var my_dir, percentage_par, percentage_perp, ray_length;
      my_dir = this.getDirection().normalize();
    ...
    Line.prototype.containsEndPoint = function(pts) {
      var dist1, dist2, i, len, pt;
      for (i = 0, len = pts.length; i < len; i++) {
        pt = pts[i];
        dist1 = pt.clone().sub(this._p1).length();
        dist2 = pt.clone().sub(this._p2).length();
        if (dist1 < BT2D.Constants.INTERSECTION_EPSILON || dist2 < BT2D.Constants.INTERSECTION_EPSILON) {
          return true;
        }
